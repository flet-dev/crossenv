...
class CrossEnvBuilder(venv.EnvBuilder):
    ...
    *always* the current interpreter.) When we refer to `host-pytohn`, we mean
    ...
    def find_sysconfig_data(self, paths):
        ...
        sysconfig_paths = sorted(sysconfig_paths, key=lambda x: len(x.split(".", 1)[1]))
        ...
        for path in sysconfig_paths:
            ...
            elif self.host_sysconfigdata.build_time_vars != syscfg.build_time_vars:
                logger.error(
                    "Conflicting build info in %s and %s",
        ...
        if not self.host_sysconfigdata:
            logger.error(
                "Cannot find _sysconfigdata*.py. Looked in %s", ", ".join(paths)
            )
    ...
    def find_host_python(self, host):
        ...
        if not os.path.exists(host):
            ...
            raise ValueError("Expected a path to a Python executable. " "Got %s" % host)
        else:
        ...
        if self._is_python_source_dir(self.host_project_base):
            ...
            try:
                ...
                raise IOError(
                    "Cannot read %s: Build the host Python first " % s
                ) from None
        ...
        if len(self.host_cc) > 1:
            ...
            logger.warning("This can cause issues for modules that don't " "expect it.")
            logger.warning(
                "Consider setting CC='%s' and CFLAGS='%s'",
                self.host_cc[0],
                " ".join(self.host_cc[1:]),
            )
        ...
        if len(self.host_cxx) > 1:
            ...
            logger.warning("This can cause issues for modules that don't " "expect it.")
            logger.warning(
                "Consider setting CXX='%s' and CXXFLAGS='%s'",
                self.host_cxx[0],
                " ".join(self.host_cxx[1:]),
            )
    ...
    def find_compiler_info(self):
        ...
        if res.returncode == 0 and found_triple:
            ...
            if (
                ...
                logger.warning(
                    "The cross-compiler (%r) does not appear to be "
    ...
    def get_uname_info(self):
        ...
        if self.macosx_deployment_target:
            try:
                ...
                raise ValueError(
                    "Unexpected value %s for MACOSX_DEPLOYMENT_TARGET"
                    % self.macosx_deployment_target
                )
            if major == 10:
                ...
                raise ValueError(
                    "Unexpected major version %s for MACOSX_DEPLOYMENT_TARGET" % major
                )
        ...
        if self.host_sys_platform == "darwin":
            self.sysconfig_platform = "macosx-%s-%s" % (
                self.macosx_deployment_target,
                self.host_machine,
            )
        ...
        self.sysconfig_ext_suffix = self.host_sysconfigdata.build_time_vars[
            "EXT_SUFFIX"
        ]
    ...
    def make_build_python(self, context):
        ...
        context.build_env_exe = os.path.join(context.build_bin_path, context.python_exe)

        # What is build-python's sys.path?
        out = subprocess.check_output(
            [context.build_env_exe, "-c", r"import sys; print('\n'.join(sys.path))"],
            universal_newlines=True,
        ).splitlines()
    ...
    def _build_pip_is_unbundled(self, context):
        ...
        bundled_module = os.path.join(
            context.build_env_dir,
            "lib",
            pyver,
            "site-packages",
            "pip",
    ...
    def make_cross_python(self, context):
        ...
        context.cross_env_exe = os.path.join(context.cross_bin_path, context.python_exe)
        ...
        pyver = "python" + sysconfig.get_config_var("py_version_short")
        context.cross_site_lib_path = os.path.join(
            context.cross_lib_path, pyver, "site-packages"
        )
        ...
        xopt = subprocess.run(
            [context.build_env_exe, "--help-xoptions"],
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
        )
    ...
    def copy_and_patch_sysconfigdata(self, context):
        ...
        # cross compiling.
        build_time_vars["BUILD_GNU_TYPE"] = sysconfig.get_config_var("BUILD_GNU_TYPE")
    ...
    def post_setup(self, context):
        ...
        tmpl.update(locals())
        utils.install_script(
            "cross-expose.py.tmpl", os.path.join(context.bin_path, "cross-expose"), tmpl
        )
...
def parse_env_vars(env_vars):
    ...
    for spec in env_vars:
        ...
        try:
            ...
            raise ValueError(
                "Invalid variable %r. Must be in the form " "NAME=VALUE" % spec
            )
...
def main():
    ...
    parser.add_argument(
        "--version", action="version", version="crossenv %s" % __version__
    )
    parser.add_argument(
        "HOST_PYTHON",
    ...
    parser.add_argument(
        "ENV_DIR", nargs="+", help="""A directory to create the environment in."""
    )
